...
**Copying text** from an embedded pane: kommand0 captures the mouse for its own UI (tree clicks, tab switching, pane focus), which suppresses the terminal's native drag-to-select — and tmux's `mouse on` adds a second capturing layer. Hold **Shift while dragging** to bypass all of it and use the terminal's native selection (most terminals also offer `Option`/`Alt`+drag for block selection). Note that this selects the literal screen, so a selection spanning panes picks up tree text and borders too.
