...
fork checkout that also has an `upstream` can never create the branch on the
wrong repo; the flip side is that an issue living only on `upstream` is reported
as not found. An issue **URL** must point at that same `origin` repo. If
`origin` isn't an `owner/repo` URL (a local path, a `host:port` remote, or no
`origin` at all), kommand0 leaves the targeting to gh and refuses URL refs.
